...
class LecturerView(APIView):
    ...
    def put(self,request,format=None):
        ...
        try:
            ...
            data = request.data
            data['classLists'] = obj.classLists
            serialize = self.serializer_class(obj,data=data)
            if serialize.is_valid():
                serialize.save()
...
class ExcelFileUpload(APIView):
    ...
    def post(self,request,format=None):
        file_details = request.data['excelFile']
        wb = openpyxl.load_workbook(file_details)
        ...
        list_of_students = [{"ID":excel_data[i][0],"first":excel_data[i][1],'last':excel_data[i][2],'email':excel_data[i][3],'dob':excel_data[i][4],'course':excel_data[i][5],'class':excel_data[i][6]} for i in range(1,len(excel_data))]
        ...
        for student in list_of_students:
            ...
            is_exist = Student.objects.filter(student_id=stu_id,email=email)
            if not is_exist:
                try:
                    is_course = Course.objects.filter(name=course)
                    if not is_course:
                        course = Course.objects.create(name=course,code=f'{course}-{random.randint(0,9)}')
                        course.semesters.add = Semester.objects.all()
                    course = Course.objects.get(name=course)

                    is_class = Class.objects.filter(number=st_class)
                    if not is_class:
                        Class.objects.create(number=st_class,course=course,lecturer_details='')

                    class_obj = Class.objects.get(number=st_class)


                    st_obj = Student.objects.create(student_id=stu_id,email=email,first_name=first,last_name=last,DOB=dob)
                    today = datetime.now()
                    StudentEnrollment.objects.create(student_id=st_obj,class_id=class_obj,enrolTime=today,gradeTime=today)
                except Exception as err:
                    print('Error on excel ',err)

        return  Response({'success':'Excel file infomation has saved'},status=status.HTTP_200_OK)
